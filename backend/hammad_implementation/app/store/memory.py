...
class AppState:
    ...
    def reset(self) -> None:
        ...
        # Seed a few deterministic BastionBot conversations.
        base_ts = now.isoformat().replace("+00:00", "Z")
        self.conversation_messages = {
            "conv-1": [
                BotMessage(id="m-1-1", role="USER", content="What does ALT-0047 indicate?", timestamp=base_ts),
                BotMessage(id="m-1-2", role="BOT", content="It suggests... [mock response].", timestamp=base_ts),
            ],
            "conv-2": [
                BotMessage(id="m-2-1", role="USER", content="Explain T0814 briefly.", timestamp=base_ts),
                BotMessage(id="m-2-2", role="BOT", content="T0814 covers... [mock response].", timestamp=base_ts),
            ],
        }
    ...
    # --- Query/Action endpoints (Hammad scope) ---
    ...
    def get_conversation_history(self, conversation_id: str) -> list[BotMessage]:
