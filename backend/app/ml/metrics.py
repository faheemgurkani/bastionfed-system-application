...
def _repo_root() -> Path:
    # backend/app/ml/metrics.py -> parents[3] = repo root
...
def get_model_metrics() -> dict[str, Any]:
    """
    Compute offline DNN metrics from FL/sample_data.csv.
    ResNet / MetaFusion require the image dataset.
    """
    ...
    if not sample_csv.exists():
        return {"available": False, "message": f"sample CSV not found: {sample_csv}"}
    ...
    if not out["models"]:
        out["available"] = False
        out["message"] = "No evaluatable DNN weights found under FL/weights."
